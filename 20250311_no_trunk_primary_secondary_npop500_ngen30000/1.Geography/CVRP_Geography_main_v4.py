...
try:
    ...
    # 対称行列をCSVファイルに保存
    symmetric_matrix_df = pd.DataFrame(symmetric_matrix, index=travel_time_matrix.index, columns=travel_time_matrix.columns)
    symmetric_matrix_df.to_csv("omaezaki_symmetric_travel_time_matrix_no_trunk_primary.csv")
...
try:
    ...
    # 対称行列をCSVファイルに保存
    symmetric_matrix_df = pd.DataFrame(symmetric_matrix, index=travel_time_matrix.index, columns=travel_time_matrix.columns)
    symmetric_matrix_df.to_csv("omaezaki_symmetric_travel_time_matrix_no_trunk_primary_secondary.csv")
